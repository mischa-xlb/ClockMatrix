...
// Reverse the bit order of a byte (mirror pixels left↔right).
...
static void flush_row(uint8_t row_reg)
{
    ...
    for (int i = 0; i < MAX7219_NUM_MODULES; i++) {
        ...
        int mod = i;
#else
        int mod = (MAX7219_NUM_MODULES - 1) - i;
#endif
#if DISPLAY_ROTATE_180
        mod = (MAX7219_NUM_MODULES - 1) - mod;   // flip module order
        buf[i * 2]     = row_reg;
        buf[i * 2 + 1] = bit_reverse(fb[mod][7 - row]); // flip rows + mirror bits
#else
        buf[i * 2]     = row_reg;
